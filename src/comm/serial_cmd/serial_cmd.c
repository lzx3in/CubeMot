 ...
 *
 * RX thread: polls UART, parses frames, publishes to msghub
 * TX thread: subscribes to msghub, encodes frames, writes UART
 *
 * Uses Zephyr UART polling API (simple, no DMA/IRQ needed).
 * USART2 on NUCLEO-G431RB: PA2(TX), PA3(RX) → Arduino connector
 */
...
static void ring_push(uint8_t byte)
{
    ...
    if (rx_head == rx_tail) {
        rx_tail = (rx_tail + 1) % RX_RING_SIZE;  // Drop oldest
    }
}
...
/**
 * @brief  Parse a complete frame from ring buffer
 * @return 0 on success, -1 if not enough data or CRC error
 */
static int parse_frame_from_ring(parsed_frame_t *frame)
{
    size_t avail = ring_available();
    
    // Need at least header (4 bytes) to know length
    if (avail < 4) return -1;
    
    // Check frame start
    if (ring_peek(0) != SERIAL_FRAME_HEAD_0 || ring_peek(1) != SERIAL_FRAME_HEAD_1) {
        // Skip one byte and let caller retry
        ring_discard(1);
        return -1;
    }
    ...
    if (frame->len > SERIAL_MAX_PAYLOAD) {
        ring_discard(2);  // Skip past header, retry
        return -1;
    }
    
    size_t frame_len = 4 + frame->len + 1;  // head(2) + cmd(1) + len(1) + payload + crc(1)
...
/**
 * @brief  Encode a response frame into tx_buf
 * @return Frame length
 */
...
/**
 * @brief  Send frame via UART (blocking)
 */
static void uart_send(size_t frame_len)
{
...
{
    k_sleep(K_MSEC(300));  // Wait for UART + other modules
    LOG_INF("Serial RX thread started");
    ...
    while (1) {
        ...
        k_usleep(500);  // ~2kHz polling (low latency for commands)
    }
}
    ...
    while (1) {
        ...
        if (++status_counter >= 10) {
            ...
            if (updated) {
                ...
                size_t frame_len = encode_frame(RSP_ID_STATUS, payload, 2);
                uart_send(frame_len);
            }
        }
        
        // Publish TELEMETRY @ 20Hz
        if (++telemetry_counter >= 5) {
            ...
            if (updated) {
                ...
                size_t frame_len = encode_frame(RSP_ID_TELEMETRY, payload, 20);
                uart_send(frame_len);
            }
        }
        ...
        if (updated) {
            ...
            size_t frame_len = encode_frame(RSP_ID_MOTOR, payload, 6);
            uart_send(frame_len);
        }
        
        k_msleep(10);  // 100Hz base rate
    }
}
...
int serial_cmd_init(void)
{
    ...
    LOG_INF("Serial command init: USART1 @ 115200 baud");
    return 0;
}
