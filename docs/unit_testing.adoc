...
使用 CMake Preset 时，构建目录结构：

[source]
----
build/
├── test-debug/             # test-debug Preset 输出
│   ├── tests/
│   │   └── cubemot-test    # 测试可执行文件
│   └── ...
└── test-release/           # test-release Preset 输出
    └── ...
----
...
项目已配置 CMake Presets，支持一键配置、构建和运行测试。
...
| `test-debug` | PC 单元测试（Debug 模式）
| `test-release` | PC 单元测试（Release 模式）
|===
...
# 或直接运行测试可执行文件
./build/test-debug/tests/cubemot-test
----
...
----
# 配置
cmake -DBUILD_TESTING=ON -B build/test-debug
...
# 显示详细输出
./build/test-debug/tests/cubemot-test --gtest_brief=0

# 运行特定测试
./build/test-debug/tests/cubemot-test --gtest_filter=LEDTest.*

# 列出所有测试
./build/test-debug/tests/cubemot-test --gtest_list_tests
----
