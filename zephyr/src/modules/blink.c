...
static void blink_thread_fn(void *arg1, void *arg2, void *arg3)
{
    ...
    for (;;) {
        bool updated = false;
        if (msghub_subscriber_check(s_button_sub, &updated) == MSGHUB_OK && updated) {
            button_event_t msg;
            if (msghub_receive(s_button_sub, &msg) == MSGHUB_OK && msg.pressed) {
        ...
        if (count > 0) {
            ...
            count = s_blink_periods_ms[s_blink_freq_index] / 50;
            led_toggle(CUBEMOT_DEVICE_LED_0);
        }
        k_msleep(50);
    }
}
